...
export default function Mechanism() {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ...
        { opacity: 0, y: 40 },
        {
          ...
          ease: "power3.out",
          stagger: 0.09,
          scrollTrigger: { trigger: ".mech-grid", start: "top 85%" },
        }
      );
    }, scope);
    return () => ctx.revert();
  ...
  return (
    <section ref={scope} id="mechanism" className="mx-auto max-w-6xl px-6 py-28">
      ...
      <div className="mech-grid mt-14 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        {STEPS.map((s, i) => (
          <div key={s.t} className="relative flex items-stretch">
            <Card className="mech-card flex-1 border-border/80 bg-card/60 backdrop-blur-sm transition-all hover:border-primary/40 hover:shadow-[0_0_40px_-14px_hsl(248_75%_64%_/_0.4)]">
