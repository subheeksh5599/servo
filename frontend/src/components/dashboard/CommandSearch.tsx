...
const DASHBOARD_COMMANDS: CommandAction[] = [
  { id: "orders", label: "Orders", icon: <ListChecks className="h-4 w-4 text-indigo" />, description: "standing orders", short: "O", end: "view", view: "orders" },
  ...
  { id: "settings", label: "Settings", icon: <Settings className="h-4 w-4 text-mist" />, description: "contracts · env", short: "S", end: "view", view: "settings" },
  { id: "new-order", label: "New order", icon: <Plus className="h-4 w-4 text-indigo" />, description: "pay once · runs forever", short: "N", end: "create", newOrder: true },
];
...
export default function CommandSearch({
  ...
  return (
    <div className="relative w-full">
      <div className="flex items-center gap-2 rounded-md border border-white/7 px-2.5 py-1.5">
      ...
      <AnimatePresence>
        {isFocused && filtered.length > 0 && (
          <motion.div
            ...
            animate="show"
            exit="exit"
            className="absolute left-0 right-0 top-full z-50 mt-1.5 overflow-hidden rounded-lg border border-white/10 bg-rail shadow-2xl shadow-black/50"
          >
            {filtered.map((cmd, i) => (
              <motion.button
                key={cmd.id}
                ...
                className={cn(
                  ...
                  activeIndex === i ? "bg-indigo/15" : "hover:bg-white/5"
                )}
              >
                <div className="flex items-center gap-2.5">
                  <span aria-hidden="true">{cmd.icon}</span>
                  <span className="font-body text-[13px] font-medium text-ink">{cmd.label}</span>
                  {cmd.description && (
                    <span className="hidden font-body text-[11px] text-mist sm:block">
                      {cmd.description}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  {cmd.short && (
                    <span className="font-mono text-[10px] text-mist">{cmd.short}</span>
                  )}
                  {cmd.end && (
                    <span className="rounded border border-white/10 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-mist">
                      {cmd.end}
                    </span>
                  )}
                </div>
            ...
            <div className="border-t border-white/7 px-3 py-1.5">
              <div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-widest text-mist/70">
